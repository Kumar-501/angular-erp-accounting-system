...
export class SaleSummaryComponent implements OnInit, OnDestroy {
  ...
  exportAsExcel(): void {
    ...
    // Prepare data for export, matching the visible columns
    const data = this.filteredSummaries.map(summary => {
        const row: any = {};
        visibleColumns.forEach(col => {
            switch(col.field) {
                case 'service.name':
                    row[col.header] = summary.service.name;
                    break;
                case 'service.description':
                    row[col.header] = summary.service.description || 'N/A';
                    break;
                case 'salesCount':
                    row[col.header] = summary.salesCount;
                    break;
                case 'totalValue':
                    row[col.header] = summary.totalValue;
                    break;
            }
        });
        return row;
    ...
    visibleColumns.forEach((col, index) => {
        ...
        } else if (col.field === 'salesCount') {
            totalsRow[col.header] = this.getTotalSales();
        } else if (col.field === 'totalValue') {
            totalsRow[col.header] = this.getTotalValue();
        } else {
